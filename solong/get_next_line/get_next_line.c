...
/*   Updated: 2023/07/03 10:55:47 by mcenci           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*gnl_breaker(char	*final, char	*buff)
{
	...
	while (buff[j] && buff[j - 1] != '\n')
		j++;
	new = ft_calloc_get (i + j + 1, sizeof(char));
	if (final != NULL)
		ft_strcpy(new, final);
	j = 0;
	while (buff[j] && buff[j] != '\n')
...
int	get_buffed(char **final, char **remind, int fd)
{
	...
	buff = ft_calloc_get (BUFFER_SIZE + 1, 1);
	while ((read (fd, buff, BUFFER_SIZE)) > 0)
	{
		if (ft_strchr_get(buff, '\n'))
		{
			*final = gnl_breaker(*final, buff);
			nl = (ft_strchr_get(buff, '\n')) + 1;
			if (nl[0])
				*remind = ft_strdup_get(nl);
			if (buff)
				free(buff);
			return (1);
		}
			...
			*final = gnl_breaker(*final, buff);
		ft_memset_get(buff, 0, BUFFER_SIZE);
	}
	free (buff);
