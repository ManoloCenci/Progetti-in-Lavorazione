...
/*   Updated: 2023/07/08 18:37:41 by mcenci           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int check_wall_rows(my_map *map)
{
	...
	i = 1;
	if ((!check_only(map->map_mtx[0], '1')) || (!check_only(map->map_mtx[map->row_map - 1], '1')))
		return (0);
	while ((map->map_mtx[i]) && (i < map->row_map))
	{
		if ((map->map_mtx[i][0] != '1') || (map->map_mtx[i][map->colum_map - 1] != '1'))
			return (0);
		i++;
	}
...
int check_correct_comp(my_map *map)
{
	...
	while (map->map_mtx[i] != NULL)
	{
		...
		{
			if (map->map_mtx[i][j] != 'P' || map->map_mtx[i][j] != 'C' || map->map_mtx[i][j] != 'E' || map->map_mtx[i][j] != 'N' || map->map_mtx[i][j] != '1' || map->map_mtx[i][j] != '0')
				return (0);
			j++;
		}
		j = 0;
